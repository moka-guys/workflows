        ...
        then
            ...
            for file in "${depends_arr[@]}"
                do 
                    ...
                    echo "$state_cmd"
                    until [[ $(eval $state_cmd) =! 'in_progress' ]] ;  
                        do
                            sleep 5
                        done
                        # Cause app to fail if the analysis state is not done (meaning it is in a failed / partially_failed / terminating / terminated state)
